...
async function handleRequest(request) {
  if (request.method === 'GET') {
    ...
    <link rel="shortcut icon" href="https://i.imgur.com/j4vp4PQ.png" type="image/x-icon">
    <style>
        * {
    ...
    <body>
     ...
     <div class="shine"></div>
     <a href="https://portfolio.rxo.me" class="home-button">Home</a>
    </body>
    </html> 
    `;
    ...
    try {
      ...
      const { name, email, comment } = jsonData; // Adjust according to your JSON structure

      // Prepare SendGrid email object
      const emailData = {
        from: {
          email: 'sender@example.com', // Replace with your actual email
          name: 'Mail Sender', // Replace with your actual name
        },
        personalizations: [
          {
            to: [{ email: 'fj@rxo.me' }], // Recipient email address (array of objects)
          },
        ],
        ...
        content: [
          {
            ...
            value: `
              <b>Name:</b> ${name}<br>
              <b>Email:</b> ${email} &nbsp; (<a href="mailto:${email}">Reply</a>)<br><br>
              <b>Message:</b><br>
              ${comment}
            `,
          },
      ...
      const response = await sendEmailWithSendGrid(emailData);
...
async function sendEmailWithSendGrid(data) {
  ...
  const apiKey = 'apikey'; // Replace with your actual SendGrid API key
